...
" My added keywords
syn keyword mysqlKeyword         commit engine

syn match mysqlKeyword "[sS][tT][aA][rR][tT]\s+[tT][rR][aA][nN][sS][aA][cC][tT][iI][oO][nN]"

" Special values
syn keyword mysqlSpecial         false null true

" My added special values
syn keyword mysqlSpecial         innodb

" Strings (single- and double-quote)
syn region mysqlString           start=+"+  skip=+\\\\\|\\"+  end=+"+
syn region mysqlString           start=+'+  skip=+\\\\\|\\'+  end=+'+

" Enable spelling checking for table comments.
syn region mysqlString           start=:[cC][oO][mM]{2}[eE][nN][tT]\s*=\s*\('\|"\): skip=+\\\\\|\\+ end=:\('\|"\): contains=@Spell
...
syn keyword mysqlTodo            contained TODO FIXME XXX GOOD BAD

" Comments (c-style, mysql-style and modified sql-style)
syn region mysqlComment          start="/\*"  end="\*/" contains=@Spell
syn match mysqlComment           "#.*" contains=@Spell
syn match mysqlComment           "--\_s.*" contains=@Spell,mysqlTodo
